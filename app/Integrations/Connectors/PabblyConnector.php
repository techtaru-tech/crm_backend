...
class PabblyConnector extends BaseConnector
{
    ...
    public function pushLead(Lead $lead, array $fieldMapping = [], array $filterConfig = []): array
    {
        ...
        $url = $this->config['webhook_url'] ?? '';
        if (empty($url)) throw new \RuntimeException(self::tx('integration_connectors.not_configured.pabbly', 'Pabbly webhook URL not configured.'));
