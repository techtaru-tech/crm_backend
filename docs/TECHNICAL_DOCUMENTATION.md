...
GET  /admin/password-setup/{token}        First-time SA password
```
...
Initiated from tenant admin → Data Privacy page.
`TenantErasureService` handles full workspace deletion:
...
- PHP 8.2 CLI at `C:\xampp\php_82_backup\php.exe` (for Artisan commands)
...
Use the PHP 8.3 CLI (matches Apache's PHP):
...
The file `storage/installed.lock` must exist for the app to skip the installer. If it is missing, the app redirects all requests to `public/install.php`.

### Cron Jobs

The file `cron.php` exposes a cron endpoint secured by `CRON_SECRET`. Schedule it to run every minute:
