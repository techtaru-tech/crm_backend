...
function checkReqs()
{
    ...
    foreach ($writeDirs as $p) {
        $f = $basePath . '/' . $p;
        $paths[] = ['path' => $p, 'ok' => is_dir($f) && is_writable($f)];
    }
    $ok = $php['passed'];
...
if ($method === 'POST') {
    if ($step === 'database') {
        ...
        $d = trim($_POST['db_database'] ?? '');
        $u = trim($_POST['db_username'] ?? '');
        $pw = $_POST['db_password'] ?? '';
        $r = testDb($h, $p, $d, $u, $pw);
        if ($r === true) {
    ...
    if ($step === 'admin') {
        $n = trim($_POST['admin_name'] ?? '');
        $em = trim($_POST['admin_email'] ?? '');
        $pw = $_POST['admin_password'] ?? '';
        $c = $_POST['admin_password_confirmation'] ?? '';
        $errs = [];
        ...
        if ($errs) {
            $_SESSION['inst_err'] = implode(' ', $errs);
            $_SESSION['inst_old'] = $_POST;
            header('Location: install.php?step=admin');
            exit;
        }
    ...
    if ($step === 'run') {
        ...
        $admin = $_SESSION['inst_admin'] ?? null;
...
// ─── Session flash ──────────────────────────────────────────────────────────
$error = $_SESSION['inst_err'] ?? '';
unset($_SESSION['inst_err']);
$old   = $_SESSION['inst_old'] ?? [];
unset($_SESSION['inst_old']);
            ...
            <div class="hdr-brand">
                <div class="hdr-icon">
                    <svg viewBox="0 0 24 24">
                        ...
                        <polyline points="9.5,16.5 12,19 14.5,16.5" />
                    </svg>
                </div>
                <h1><?= e($appName) ?></h1>
            </div>
        ...
        <div class="body">
            ...
            ?>
                ...
                <div class="actions">
                    <span></span>
                    <?php if ($req['ok']): ?>
                        <a href="install.php?step=database" class="btn btn-p">Continue <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            ?>
                ...
                <form method="POST" action="install.php?step=database">
                    ...
                    <div class="actions">
                        <a href="install.php?step=requirements" class="btn btn-s"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M19 12H5M12 19l-7-7 7-7" />
                            </svg> Back</a>
                        <button type="submit" class="btn btn-p">Test &amp; Continue <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M5 12h14M12 5l7 7-7 7" />
                            </svg></button>
                    </div>
                </form>
            ...
            ?>
                ...
                <form method="POST" action="install.php?step=admin">
                    ...
                    <div class="actions">
                        <a href="install.php?step=database" class="btn btn-s"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M19 12H5M12 19l-7-7 7-7" />
                            </svg> Back</a>
                        <button type="submit" class="btn btn-p">Continue <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M5 12h14M12 5l7 7-7 7" />
                            </svg></button>
                    </div>
                </form>
            ...
            ?>
                ...
                <form method="POST" action="install.php?step=license">
                    ...
                    <div class="actions">
                        <a href="install.php?step=admin" class="btn btn-s"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M19 12H5M12 19l-7-7 7-7" />
                            </svg> Back</a>
                        <button type="submit" class="btn btn-p">Continue <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M5 12h14M12 5l7 7-7 7" />
            ...
            ?>
                ...
                <div id="done" style="display:none">
                    <div class="alert alert-ok"><?= e($appName) ?> installed successfully! Redirecting to admin panel...</div>
                </div>
                <div id="fail" style="display:none">
                    <div class="alert alert-err" id="em"></div>
                </div>

                <div class="actions" id="btns">
                    <a href="install.php?step=license" class="btn btn-s"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M19 12H5M12 19l-7-7 7-7" />
                        </svg> Back</a>
                    <button class="btn btn-p" id="ibtn" onclick="run()">Install <?= e($appName) ?> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M5 12h14M12 5l7 7-7 7" />
                        </svg></button>
                </div>
                ...
                <script>
                    async function run() {
                        ...
                        var s = [{
                                m: 'Writing configuration...',
                                p: 15
                            }, {
                                m: 'Preparing database...',
                                p: 30
                            }, {
                                m: 'Running migrations...',
                                p: 50
                            }, {
                                m: 'Seeding data...',
                                p: 65
                            }, {
                                m: 'Creating admin account...',
                                p: 80
                            }, {
                                m: 'Finalizing...',
                                p: 92
                            ...
                            iv = setInterval(function() {
                                if (i < s.length) {
                                    pm.textContent = s[i].m;
                                    pb.style.width = s[i].p + '%';
                                    i++
                                }
                            }, 1500);
                        try {
                            var r = await fetch('install.php?step=run', {
                                method: 'POST',
                                headers: {
                            ...
                            if (d.success) {
                                ...
                                pg.style.display = 'none';
                                document.getElementById('em').textContent = d.message;
