...
<head>
    ...
    <title>@yield('title', $seoTitle !== '' ? $seoTitle : $appName)</title>
    <meta name="description" content="@yield('description', $seoDescription !== '' ? $seoDescription : __('marketing.meta_default_description'))" />
    @if($seoKeywords !== '')
    ...
    <link rel="apple-touch-icon" sizes="180x180" href="{{ $brandFavicon !== '/favicon.svg' ? $brandFavicon : '/apple-touch-icon.png' }}">
    <link rel="manifest" href="/site.webmanifest">
    <meta name="theme-color" content="#4f46e5">
    <link rel="preload" as="image" href="{{ $brandFavicon }}">

    {{-- Inter font self-hosted under public/vendor/ — no Google Fonts
         CDN load.  Loaded as a <link> at the top of <head> so the
    ...
    <meta property="og:title" content="@yield('title', $seoTitle !== '' ? $seoTitle : ($appName ?? 'LeadHub'))" />
    <meta property="og:description" content="@yield('description', $seoDescription !== '' ? $seoDescription : __('marketing.meta_og_description'))" />
    <meta property="og:url" content="{{ url('/') }}" />
    <meta property="og:site_name" content="{{ $appName ?? 'LeadHub' }}" />
    <meta property="og:image" content="{{ $ogImage }}" />

    {{-- Twitter Card --}}
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="@yield('title', $seoTitle !== '' ? $seoTitle : ($appName ?? 'LeadHub'))" />
    <meta name="twitter:description" content="@yield('description', $seoDescription !== '' ? $seoDescription : __('marketing.meta_twitter_description'))" />
    ...
    {{-- Defense-in-depth: JSON_HEX_TAG escapes `<` and `>` as
         ...
         string containing `</script>` cannot close this <script> block
         early and execute arbitrary HTML/JS in the rest of the document.
         JSON_HEX_AMP / JSON_HEX_APOS / JSON_HEX_QUOT add belt-and-
         suspenders against other context-escape vectors. --}}
    <script type="application/ld+json">{!! json_encode($__jsonLdSwap($__jsonLd), JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>
    <link rel="stylesheet" href="{{ asset('css/marketing/layout.css') }}">
    @stack('head')
</head>
<body>
@php
...
<header class="nav">
    <div class="wrap nav-inner">
        <a href="/" class="brand">
            ...
            {{-- Wordmark in its own span so mobile CSS can hide it
                 at ultra-narrow widths without losing the favicon
                 touch-target. --}}
            <span>{{ $appName }}</span>
        </a>
        <nav class="nav-links">
            @if($lc->navShows('pricing'))
                {{-- Anchor to the in-page pricing section on home,
                     falls back to the standalone /pricing route on
                     other pages (browser handles the missing anchor
                     gracefully). --}}
                <a href="{{ url('/') }}#pricing" class="link">{{ __('marketing.nav.pricing') }}</a>
            @endif
            @if($lc->navShows('docs'))
                <a href="/docs" class="link">{{ __('marketing.nav_docs') }}</a>
            @endif
            @foreach($navStaticPages as $__sp)
                @if(is_object($__sp))
                    <a href="{{ url('/pages/' . $__sp->slug) }}" class="link">{{ $__sp->t('title') }}</a>
                @endif
            @endforeach
            @if($lc->navShows('sign_in'))
                <a href="/admin/login" class="link">{{ __('marketing.nav.sign_in') }}</a>
            @endif
            @if($canSignUp ?? false)
                <a href="/register" class="btn btn-primary">{{ __('marketing.nav.start_trial') }}</a>
            @else
                <a href="/admin/login" class="btn btn-primary">{{ __('marketing.nav.get_started') }}</a>
            @endif

            @if(count($__locales) > 1)
                <div class="lang-switch">
                    <button type="button" aria-label="{{ __('marketing.language') }}">
                        <span>{{ $__locales[$__locale]['flag'] ?? '🌐' }}</span>
                        <span>{{ strtoupper($__locale) }}</span>
                    </button>
                    <div class="menu">
                        @foreach($__locales as $code => $meta)
                            <a href="{{ url('/locale/' . $code) }}" class="{{ $code === $__locale ? 'active' : '' }}">
                                <span>{{ $meta['flag'] ?? '🌐' }}</span>
                                <span>{{ $meta['name'] ?? strtoupper($code) }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
            ...
            {{-- Hamburger — only renders on mobile via CSS.  Opens
                 the .nav-mobile-panel below the pill. --}}
            <button type="button" class="nav-toggle" aria-label="{{ __('marketing.nav_menu_aria') }}" aria-expanded="false" aria-controls="nav-mobile-panel">
    ...
    <div id="nav-mobile-panel" class="nav-mobile-panel" hidden>
        @if($lc->navShows('pricing'))
            <a href="{{ url('/') }}#pricing">{{ __('marketing.nav.pricing') }}</a>
        @endif
        @if($lc->navShows('docs'))
            <a href="/docs">{{ __('marketing.nav_docs') }}</a>
        @endif
        @foreach($navStaticPages as $__sp)
            @if(is_object($__sp))
                <a href="{{ url('/pages/' . $__sp->slug) }}">{{ $__sp->t('title') }}</a>
            @endif
        @endforeach
        @if($lc->navShows('sign_in'))
            <a href="/admin/login">{{ __('marketing.nav.sign_in') }}</a>
        @endif

        @if(count($__locales) > 1)
            <div class="nav-mobile-langs">
                <span class="nav-mobile-langs-label">{{ __('marketing.language') }}</span>
        ...
        <div class="foot-grid">
            @if($footerShowBrand)
                <div class="foot-brand">
                    <div class="wordmark">
                        {{-- Same $brandLogo as the nav pill so header and
                             footer marks stay identical.  Reads the
                             SA-global BrandingSettings.logo_url (falls
                             back to the bundled /favicon.svg) — see the
                             PHP block at the top of this layout (do not write
                             that directive name with a leading @ inside a Blade
                             comment — Blade parses it as a real directive). --}}
                        <img src="{{ $brandLogo }}" alt="{{ $appName }}">
                        <span>{{ $appName }}</span>
                    </div>
                    <p>{{ $footerTagline }}</p>
                    @if($footerHasSocial)
                        <div class="foot-social">
                            @if($fX !== '')
                                <a href="{{ $fX }}" target="_blank" rel="noopener" aria-label="{{ __('marketing.footer_social_x_aria') }}">
                            ...
                            @endif
                            @if($fGh !== '')
                                <a href="{{ $fGh }}" target="_blank" rel="noopener" aria-label="{{ __('marketing.footer_social_github_aria') }}">
                            ...
                            @endif
                            @if($fLi !== '')
                                <a href="{{ $fLi }}" target="_blank" rel="noopener" aria-label="{{ __('marketing.footer_social_linkedin_aria') }}">
                            ...
                            @endif
                            @if($fFb !== '')
                                <a href="{{ $fFb }}" target="_blank" rel="noopener" aria-label="{{ __('marketing.footer_social_facebook_aria') }}">
                                    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                </a>
                            @endif
                            @if($fIg !== '')
                                <a href="{{ $fIg }}" target="_blank" rel="noopener" aria-label="{{ __('marketing.footer_social_instagram_aria') }}">
                            ...
                            @endif
                            @if($fYt !== '')
                                <a href="{{ $fYt }}" target="_blank" rel="noopener" aria-label="{{ __('marketing.footer_social_youtube_aria') }}">
            ...
            @if($footerShowNavLinks)
                <div class="foot-col">
                    <h4>{{ __('marketing.footer_col_product') }}</h4>
                    <ul>
                        @if($lc->navShows('pricing'))<li><a href="{{ url('/') }}#pricing">{{ __('marketing.footer.pricing') }}</a></li>@endif
                        @if($lc->navShows('docs'))<li><a href="/docs">{{ __('marketing.footer_documentation') }}</a></li>@endif
                        @if($lc->navShows('sign_in'))<li><a href="/admin/login">{{ __('marketing.footer.sign_in') }}</a></li>@endif
                        @if($canSignUp ?? false)<li><a href="/register">{{ __('marketing.nav.start_trial') }}</a></li>@endif
                    </ul>
                </div>
            @endif

            @if($footerShowStatic && $footerStaticPages->isNotEmpty())
                <div class="foot-col">
                    <h4>{{ __('marketing.footer_col_resources') }}</h4>
                    <ul>
                        @foreach($footerStaticPages as $__fp)
                            @if(is_object($__fp))
                                <li><a href="{{ url('/pages/' . $__fp->slug) }}">{{ $__fp->t('title') }}</a></li>
                            @endif
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="foot-col">
                <h4>{{ __('marketing.footer_col_company') }}</h4>
                <ul>
                    <li><a href="/pages/about">{{ __('marketing.footer_about') }}</a></li>
                    <li><a href="/pages/contact">{{ __('marketing.footer_contact') }}</a></li>
                    <li><a href="/pages/terms">{{ __('marketing.footer_terms') }}</a></li>
                    <li><a href="/pages/privacy">{{ __('marketing.footer_privacy') }}</a></li>
                </ul>
            </div>
        </div>

        <div class="foot-bottom">
            {{-- Fix note: footer copy is SA-supplied. Prior stripTags allowlist
                 kept ALL attributes on <a>/<br> — including onclick=, onmouseover=,
                 and href="javascript:". Apply the same multi-layer regex defense
                 used in landing/sections/html  so a compromised SA can't
                 inject scripts OR event handlers OR javascript: URIs into every
            ...
            @endphp
            <span class="copy">{!! $safeFooter !!}</span>
            <div class="legal">
                @if($lc->navShows('pricing'))<a href="{{ url('/') }}#pricing">{{ __('marketing.footer.pricing') }}</a>@endif
                <a href="mailto:hello@{{ parse_url(config('app.url'), PHP_URL_HOST) ?: 'example.com' }}">{{ __('marketing.footer_support') }}</a>
            </div>
        </div>
    </div>
</footer>

{{-- Nav elevation on scroll + mobile hamburger — shared across every public page. --}}
<script src="{{ asset('js/views/marketing/layout.js') }}" defer></script>
